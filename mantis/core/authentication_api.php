...
// phpbb_integration - Include file authentication_api_phpbb.php
require_once( dirname( __FILE__ ) . DIRECTORY_SEPARATOR . 'authentication_api_phpbb.php');
...
function auth_does_password_match( $p_user_id, $p_test_password ) {
	...
	// phpbb_integration - Checks the password using phpbb's functions.
	if (config_get('phpbb_integration') == ON) {
		$phpbb = new Phpbb3Integration();
		if($phpbb->phpbb_check_hash($p_test_password, $t_password)) {
			return true;
		}
		return false;
	}
 ...
 */
 function auth_process_plain_password( $p_password, $p_salt = null, $p_method = null ) {
 	...
 	// phpbb_integration - This should never be called with the integration piece, it should 
	// branch off before here and use phpbb's implementation.  
	if (config_get('phpbb_integration') == ON) {
		return utf8_substr( "invalid", 0, DB_FIELD_SIZE_PASSWORD );
	}
